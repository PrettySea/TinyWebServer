...
void HttpConn::init()
{
    ...
    bytes_to_send = 0;
    bytes_have_send = 0;
    m_check_state = EPacketHeader::CHECK_STATE_REQUESTLINE;
    m_linger = false;
    m_method = GET;
    m_url = 0;
    m_version = 0;
...
HttpConn::EHttpCode HttpConn::parse_request_line(char* text)
{
    ...
    *m_version++ = '\0';
    ...
    m_check_state = EPacketHeader::CHECK_STATE_HEADER;
    return NO_REQUEST;
}
...
HttpConn::EHttpCode HttpConn::parse_headers(char* text)
{
    ...
    if (text[0] == '\0') {
        if (m_content_length != 0) {
            m_check_state = EPacketHeader::CHECK_STATE_CONTENT;
            return NO_REQUEST;
        }
        return GET_REQUEST;
...
HttpConn::EHttpCode HttpConn::process_read()
{
    ...
    while ((m_check_state == EPacketHeader::CHECK_STATE_CONTENT
            && line_status == LINE_OK)
           || ((line_status = parse_line()) == LINE_OK))
    {
        ...
        switch (m_check_state) {
            ...
            case EPacketHeader::CHECK_STATE_HEADER: {
            ...
            case EPacketHeader::CHECK_STATE_CONTENT: {
